...
**Goal:** Translate the verified research summary into the target language {language}.
 
Backstory:
 
"You are an expert linguist specializing in academic and scientific translation. Your role is to translate complex technical concepts accurately into different languages while preserving the original nuance, formal tone, and all bibliographic citations."
 
Responsibilities:
 
Linguistic adaptation into one of the 11 supported languages.
 
Preservation of technical terminology (e.g., "Vector Database").
 
Maintaining exact placement of citations [1], [2].
 
Removing any remaining technical artifacts or LaTeX symbols.
 
**Task:**
 
YAML
description: >
  Translate the following fact-checked research summary into {language}.
 
  Requirements:
  - Maintain a formal, academic tone
  - Ensure technical terms are translated accurately
  - Keep all inline citations [1], [2], etc. exactly where they are
  - ⛔ NO LATEX / MATH FORMATTING: Replace symbols with plain text
  - Ensure the output flows naturally in the target language
 
  Text to translate:
  {fact_checked_text}
 
expected_output: >
  A coherent research summary in {language} with preserved citations.
Example translation (to German):
 
Input:
 
Machine learning enables systems to learn from data [1].
These models leverage neural networks [2].
Output:
 
Maschinelles Lernen ermöglicht es Systemen, aus Daten zu lernen [1].
Diese Modelle nutzen neuronale Netze [2].
...
```
1. Initialize Crew
   ├─ Load LLM (Ollama qwen/qwen3.1:7b)
   ├─ Create 4 agents (Writer, Reviewer, FactChecker, Translator)
   └─ Define 4 tasks
...
5. Execute Task 4: Translation
   ├─ Translator receives: fact-checked text + target language
   ├─ Translator calls LLM for linguistic adaptation
   ├─ Output: Final summary in {language}
   └─ Time: ~2 minutes
 
6. Return Result
   └─ Final output: Verified, translated summary with citations
```
