...
QUESTION = "Give me a concise 6–8 sentence overview across these docs and cite filenames as [1], [2], ..."
...
def load_docs() -> List[Document]:
    ...
    if pdfs:
        docs += PyPDFToDocument().run(sources=[str(p) for p in pdfs])["documents"]
    if txts:
        docs += TextFileToDocument(encoding="utf-8").run(
            sources=[str(t) for t in txts]
        )["documents"]
...
def build_store(
    ...
    """Split documents into chunks, embed them, and return an in-memory retriever."""
...
def call_ollama(context: List[Document], question: str, model_name: str) -> str:
    """Send a summarized question–answer request to Ollama with inline citations."""
    uniq, lines = [], []
    for d in context:
